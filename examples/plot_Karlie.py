...
#Open data
fitsname = "/home/amarchal/Projects/deconv/examples/data/MeerKAT/result_chan_0000_to_0300.fits"
hdu = fits.open(fitsname)
hdr = hdu[0].header
cube = hdu[0].data[100:250]
...
writer = imageio.get_writer(pathout + 'movie4.mp4', fps=10)
...
contours = ax.contour(pb_mean, linestyles="--", levels=[0.2, 0.3], colors=["w", "w"])

# Create the colorbar once before the loop
img = ax.imshow(cube[0] * mask, vmin=-2, vmax=5, origin="lower", cmap="viridis")
...
for i in tqdm(np.arange(len(cube))):
    ...
    # smoothed_data = gaussian_filter(cube[i] * mask, sigma=1.5)
    # img.set_data(smoothed_data)
