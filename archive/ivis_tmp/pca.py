...
class PCASpectralModel(BaseModel):
    """
    Spectral basis model driven by a fitted PCA object.

    The optimized tensor contains one spatial coefficient map per retained
    spectral atom. The loss and forward model are accumulated directly in
    visibility space as ``sum_k psi_k(v) H_v(a_k)``.
    """
    ...
    def __init__(
        ...
        if not hasattr(pca, "components_"):
            raise TypeError("pca must be a fitted PCA-like object with components_.")

        components = np.asarray(pca.components_, dtype=np.float32)
        if components.ndim != 2:
            raise ValueError("pca.components_ must have shape (n_components, nchan).")

        max_k, nchan = components.shape
        if k is None:
            k = max_k
        k = int(k)
        if k <= 0:
        ...
        if k > max_k:
            raise ValueError(f"k={k} exceeds available PCA components ({max_k}).")

        if mean_spec is None:
            if hasattr(pca, "mean_"):
                mean_spec = np.asarray(pca.mean_, dtype=np.float32)
            elif use_mean:
                raise ValueError("use_mean=True requires mean_spec or pca.mean_.")

        if mean_spec is not None:
            mean_spec = np.asarray(mean_spec, dtype=np.float32)
            if mean_spec.shape != (nchan,):
                raise ValueError(
                    f"mean_spec must have shape ({nchan},), got {tuple(mean_spec.shape)}"
                )

        atoms = []
        if use_mean:
            atoms.append(mean_spec)
        atoms.extend(components[:k])
        ...
        self._basis_np = np.stack(atoms, axis=0).astype(np.float32, copy=False)
        self._basis_cache = {}
    ...
    def reconstruct_cube(self, x, device=None, return_numpy=False):
        """
        Reconstruct the spectral cube from PCA coefficient maps.

        Parameters
        ----------
        x : np.ndarray or torch.Tensor
            Coefficient maps with shape (nbasis, ny, nx).
        device : str or torch.device, optional
            Target device. If omitted, keep the current tensor device or use CPU.
        return_numpy : bool, optional
            If True, return a NumPy array on CPU. Otherwise return a torch.Tensor.
    ...
    def reconstruct_cube_from_coeffs(self, coeffs, device=None, return_numpy=True):
        """
        Convenience wrapper for reconstructing a cube from solver output.
        """
    ...
    def forward(
        ...
        if nchan != self.nchan:
            raise ValueError(f"Expected nchan={self.nchan} from PCA basis, got {nchan}.")
